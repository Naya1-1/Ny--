(function (global) {
  ...
  Ny.CodeGen = Ny.CodeGen || (function () {
    ...
    // Background builders (delegate to Ny.Background when available to unify source of truth)
    function buildBgLayersHTML(layers, customization) {
      customization = customization || (State.customization || {});
      try {
        if (Ny && Ny.Background && typeof Ny.Background.buildBgLayersHTML === 'function') {
          return Ny.Background.buildBgLayersHTML(layers, customization);
        }
      } catch (_delegateErr) {}
      // Fallback: local simplified builder (kept to avoid breaking export if Background module is unavailable)
      try {
    ...
    function buildBgComponentsHTML(components) {
      try {
        if (Ny && Ny.Background && typeof Ny.Background.buildBgComponentsHTML === 'function') {
          return Ny.Background.buildBgComponentsHTML(components);
        }
      } catch (_delegateErr2) {}
      // Fallback: local simplified builder to keep export functional if Background module is missing
      try {
    ...
    function computeWrapperAttrs(S) {
      ...
      if (cfg.layout !== 'stacked' && cfg.layout !== 'center' && cfg.layout !== 'two-column') {
    ...
    function buildReplacementHTML(state, options) {
      try {
        ...
        var VM = (State && typeof State.getViewModel === 'function') ? State.getViewModel() : null;
        var __layers = (cfg.bgMode === 'layers') ? ((VM && VM.background && Array.isArray(VM.background.layers)) ? VM.background.layers : cfg.bgLayers) : [];
        var __comps  = (cfg.bgMode === 'layers') ? ((VM && VM.background && Array.isArray(VM.background.components)) ? VM.background.components : cfg.bgComponents) : [];
        var bgLayersHTML = (cfg.bgMode === 'layers') ? buildBgLayersHTML(__layers, cfg) : '';
        var bgCompsHTML  = (cfg.bgMode === 'layers') ? buildBgComponentsHTML(__comps) : '';
        ...
        var bodyHTML = (items || []).map(function (it) {
          ...
          if (it.type === 'bar') {
            ...
            return ''
              ...
              + '  <div class="st-value" style="width: clamp(120px, 40vw, 160px); ' + valStyle2 + '">'
